...
VALUES (N'plinko|risk={b.risk}|rows={b.rows}|client={b.client_seed}', NULL);
...
EXEC dbo.usp_Apuesta_Colocar
  ...
  @Importe={b.amount},
  @IdTipoAsiento=(SELECT IdTipo FROM dbo.TiposTransaccion WHERE Nombre=N'apuesta'),
  @IdCuentaCasaJuego=(SELECT IdCuenta FROM dbo.Cuentas WHERE IdUsuario=(SELECT IdUsuario FROM dbo.Usuarios WHERE Email=N'house@casino.local') AND IdMoneda={b.moneda_id}),
  @Detalle=@det,
